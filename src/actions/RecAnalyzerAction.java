...
import processing.SoundAnalyzer;
import processing.RecordingAnalyzer;
import processing.WavFile;
import processing.Word;
import view.MainScreen;

public class RecAnalyzerAction implements EventHandler<ActionEvent> {
	...
	public void handle(ActionEvent arg0) {
		if (MainScreen.get().TP.getSelectionModel().getSelectedItem() == MainScreen.get().Database) {
			...
			for (Word word : analyzer.words) {
				...
				word.name = MainScreen.get().SwordsInList.size() + 1 + "";
	...
	public void saveWord(SoundAnalyzer analyzer, Word word, String file) {
		try {
